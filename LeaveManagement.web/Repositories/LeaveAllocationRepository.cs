...
namespace LeaveManagement.web.Repositories
{
    public class LeaveAllocationRepository : GenericRepository<LeaveAllocation>, ILeaveAllocationRepository
    {
        ...
        public async Task<EditLeaveAllocationVM> GetEmployeeAllocations(int id)
        {
            var allocation = await context.LeaveAllocations
                .Include(q => q.LeaveType)
                .ProjectTo<EditLeaveAllocationVM>(configurationProvider)
        ...
        public async Task LeaveAllocation(int leaveTypeId)
        {
            ...
            foreach(var employee in employeesWithNewAllocations)
            {
                ...
                await emailSender.SendEmailAsync(employee.Email, $"Leave Allocation Period for {period}", $"Your {leaveType.Name} " +
                    $"has been posted for the period of {period}. You have been given {leaveType.DefaultDays}"
                    );
            }
        }

        public async Task<bool> UpdateEmployeeAllocations(EditLeaveAllocationVM model)
        {
